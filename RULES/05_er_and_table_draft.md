...
現在のMVP実装は Spreadsheet を保存先に使うが、将来のRDB移行を見据えて PostgreSQL 系に寄せた命名と制約意図で整理します。
...
## 9. Spreadsheet 実装への読み替え

MVPではDBの代わりに Spreadsheet のシートを使う想定のため、各テーブルは原則 1 シートへ対応づける。

読み替え方針:

- PK は先頭付近に `*_id` カラムとして持つ
- FK制約はDBではなくアプリ側バリデーションで担保する
- `unique` 制約は保存前チェックで担保する
- `index` は物理 index ではなく、検索回数削減と一括読み込み戦略で吸収する
- `jsonb` 想定列は Spreadsheet では JSON 文字列として保持する
